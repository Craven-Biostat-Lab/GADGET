...
from models import Metabolite, MetaboliteAbstract, Abstract
...
def metabolitesearch(request):
    """Does the actual search for the metabolite search.  Given a keyword query,
    ...
    metabolites relevent to the query via the index and database, and returns the 
    ...
    # No homologs for metabolite view
    geneabstract_tablename = 'metabolite_abstract'
    abstract_col = 'abstracts'
    ...
    INNER JOIN `metabolite` g
    ON g.hmdb_id = a.metabolite_hmdb_id
...
def searchresponse(validresult, download=None, errmsg=None, results=[], genes=[], geneop=None, pvals=[], offset=0, orderby=None, query=None, limit=None, usehomologs=None, species=None, query_abstract_count=None, abstracts=None, usegenefile=None):
    ...
    if download:
        ...
        if validresult:
            resulthtml = render_to_string('metabolitelist.html', {'results': zip(results, pvals), 'pvals': pvals, 'offset': offset, 'orderby': orderby})
        else:
            resulthtml = None
